...
$(function(){
	...
	$("table#CustomerGrid").jqGrid({
		...
		colModel: [
			...
			{ label: '小区位置', name: 'customerhome.roomno.building.area.address', width: 80,align:"center" },
			...
			{ label: '电话', name: 'mobile', width: 70,align:"center" },
			{ label: '居住人口', name: 'customerhome.humancount', width:40,align:"center" },
			{ label: '入住状态', name: 'customerhome.roomno.roomstatus', width:40,align:"center" },
			{ label: '住宅类型', name: 'customerhome.roomno.roomtype', width:40,align:"center" }   
		],
		caption:"信息列表",
		viewrecords: true, 
		autowidth: true,
	...
	$.getJSON(host+"room/type/all",function(List){
		...
		if(List){
			$.each(List,function(index,res){
				$("select#typeno").append("<option value='"+index+"'>"+res.departmentcode+"</option>");
			});
		}
	});
	...
	$.getJSON(host+"room/status/all",function(List){
		...
		if(List){
			$.each(List,function(index,res){
				$("select#status").append("<option value='"+index+"'>"+res.roomstatus+"</option>");
			});
		}
	});
//	
